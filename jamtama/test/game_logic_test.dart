...
void main() {
  ...
  // mace: forward(0,1), left(-1,0), right(1,0) — same pattern as the old "boar".
  group('computeValidMoves — mace (forward, left, right)', () {
    test('Red piece in open centre returns all 3 moves', () {
      const piece = Piece(type: PieceType.student, player: Player.red, row: 2, col: 2);
      final moves = computeValidMoves(piece, mace, empty);
  ...
  group('computeValidMoves — blocking', () {
    test('Cannot land on own piece', () {
      ...
      final moves = computeValidMoves(piece, mace, [piece, blocker]);
    ...
    test('Can land on opponent piece (capture)', () {
      ...
      final moves = computeValidMoves(piece, mace, [piece, target]);
    ...
    test('All moves blocked by own pieces returns empty', () {
      ...
      final moves = computeValidMoves(piece, mace, [piece, ...blockers]);
      expect(moves, isEmpty);
    });
  });

  // dagger: all 4 diagonals — same pattern as the old "monkey".
  group('computeValidMoves — dagger (all 4 diagonals)', () {
    test('Centre piece has 4 diagonal moves', () {
      const piece = Piece(type: PieceType.student, player: Player.red, row: 2, col: 2);
      final moves = computeValidMoves(piece, dagger, empty);
      expect(moves, containsAll(pos([(3, 1), (3, 3), (1, 1), (1, 3)])));
      expect(moves.length, 4);
    });

    test('Blue dagger moves are the same squares (diagonals are symmetric)', () {
      const piece = Piece(type: PieceType.student, player: Player.blue, row: 2, col: 2);
      final moves = computeValidMoves(piece, dagger, empty);
      expect(moves, containsAll(pos([(3, 1), (3, 3), (1, 1), (1, 3)])));
      expect(moves.length, 4);
  ...
  group('computeValidMoves — card mirroring (Red vs Blue)', () {
    // longsword: forward, right, backward — same as old "ox".
    test('Red longsword from (2,2): forward=(3,2), right=(2,3), back=(1,2)', () {
      const piece = Piece(type: PieceType.student, player: Player.red, row: 2, col: 2);
      final moves = computeValidMoves(piece, longsword, empty);
      expect(moves, containsAll(pos([(3, 2), (2, 3), (1, 2)])));
      expect(moves.length, 3);
    });

    test('Blue longsword from (2,2): forward=(1,2), right=(2,1), back=(3,2)', () {
      const piece = Piece(type: PieceType.student, player: Player.blue, row: 2, col: 2);
      final moves = computeValidMoves(piece, longsword, empty);
    ...
    // saber: forward, left, backward — same as old "horse".
    test('Red saber and Blue saber are mirror images', () {
      ...
      final redMoves = computeValidMoves(redPiece, saber, empty);
      final blueMoves = computeValidMoves(bluePiece, saber, empty);
  ...
  // halberd: front-left, front-right, left, right — same as old "elephant".
  group('computeValidMoves — halberd (front-diagonals + sides)', () {
    test('Red halberd from centre has 4 moves', () {
      const piece = Piece(type: PieceType.student, player: Player.red, row: 2, col: 2);
      final moves = computeValidMoves(piece, halberd, empty);
